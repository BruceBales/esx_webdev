...
RegisterNetEvent('esx_webdev:currentlyRobbing')
AddEventHandler('esx_webdev:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_webdev:killBlip')
AddEventHandler('esx_webdev:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_webdev:setBlip')
AddEventHandler('esx_webdev:setBlip', function(position)
...
RegisterNetEvent('esx_webdev:tooFar')
AddEventHandler('esx_webdev:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_webdev:robberyComplete')
AddEventHandler('esx_webdev:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_webdev:startTimer')
AddEventHandler('esx_webdev:startTimer', function()
	...
	while true do
				...
				if not holdingUp then
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							TriggerServerEvent('esx_webdev:robberyStarted', k)
						end
					end
				end
			end
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_webdev:tooFar', store)
			end
		end
	end
end)
